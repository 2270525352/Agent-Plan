...
单窗口模式:把下面整段贴进 Codex 的 `/goal`,作为长期目标。**先在 Codex App 挂好定时审查(见 `../07-testing/CodexApp定时测试提示词.md`),再开 goal。**
...
6. 测试过 + 自审过才提交:提交信息写「<任务编号> (Codex): <简述>」;提交后 push 到专用功能分支,绝不 push 到 main / master。
...
外层有 Codex App 每 30 分钟跑的独立审查代理盯着你跑偏,它只检测、把问题写进偏离报告,不替你修。你照常干活,看到新 RED 就停。
```
