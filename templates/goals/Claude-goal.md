...
单窗口模式:把下面整段贴进 Claude 的 `/goal`,作为长期目标。**先用 Claude Code `/schedule` 挂好定时审查(见 `../07-testing/ClaudeCode定时任务提示词.md`),再开 goal。** 本项目已装护栏(见 `../10-guards/护栏说明.md`):改任务范围外的文件、动用户原话、提交信息不合格、push 到 main,都会被 hook 当场拦下——按规矩走就不会撞到。
...
7. 测试过 + 自审过 → 做一次 checkpoint:提交信息写「<任务编号> (Claude): <简述>」,push 到专用功能分支,绝不 push 到 main / master。**每个任务都要提交 + push**——外层定时审查只看得到已提交的改动,不提交就等于没让它审。
