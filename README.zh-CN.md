...
- **根目录 `AGENTS.md`** — 仓库级长期约束，新窗口第一份要读的规则文件；已有内容用 marker 合并，不覆盖。
- **根目录 `CURRENT_STATE.md`** — 动态进度与跨窗口交接，记录当前任务、写入锁、阻塞、下一步、checkpoint。
...
生成规划树前，先在目标项目根目录生成或合并协调文件：

```bash
python3 scripts/agent-plan-bootstrap.py install --project /path/to/project
```

脚本会保留已有 `AGENTS.md`，只插入或更新 `<!-- AGENT_PLAN_START --> ... <!-- AGENT_PLAN_END -->` 这块。`CURRENT_STATE.md` 不存在时创建，已存在时默认不覆盖；需要记录本次启动层更新时加 `--append-state-log`。`--force-agents` / `--force-current-state` 只在用户明确同意后使用。
...
9. 进入实现：主线 AI 先读 `AGENTS.md` 和 `CURRENT_STATE.md`，再自主跑循环；必要时按合同派发支线，合流门禁通过后再纳入主线；按 Git 检查点提交并回写 `CURRENT_STATE.md`。

## 分档

别给一个 todo CLI 生成 30 个文件。选一档，决定生成哪些文档；**防跑偏核心（根目录 `AGENTS.md`、根目录 `CURRENT_STATE.md`、真相源、AI 可读需求、总任务、任务说明+反馈+支线记录、goal、定时审查、运行时、git、护栏）每档都有**，分档只加规划深度。

| 档 | 适用 | 大致 |
| :--- | :--- | :--- |
| **lite** | 小 / 单组件 / 一次性，需求基本清楚 | ~16 个文件（含根目录启动层），只有核心。不单出架构文档（有真接口才加 `接口契约文档`）。 |
| **standard**（默认） | 一般的多模块项目 | lite + `用户强调事项`、`需求对齐检查表`、`总架构文档`（图内嵌）+ `接口契约文档`、`阶段交付计划`、`需求对齐审查`、`提交检查表`。~22 个。 |
| **full** | 大 / 多组件 / 团队交接 / 合规 | 全部模板，含展开的散文档 `总需求`、`总设计`、`架构图`、`任务依赖图`、`架构一致性审查`。~30 个。 |

> 合并冗余：lite/standard 不出「把可读结构再用大白话重写一遍」的散文档——`总需求文档`（用 `AI可读需求文档`）、`总设计 / 架构图`（mermaid 内嵌进 `总架构文档`）。full 保留展开版，给大项目和交接用。

## 输出目录

默认先在目标项目根目录生成 `AGENTS.md` / `CURRENT_STATE.md`，再生成 `docs/agent-plan/`（下面是 **full** 全量树，lite/standard 取子集，见上「分档」）：

```text
AGENTS.md          仓库级长期规则（已有文件用 marker 合并）
CURRENT_STATE.md   当前状态、写入锁、阻塞、下一步
docs/agent-plan/
...
| 目录 | 职责 |
| :--- | :--- |
| `AGENTS.md` | 根目录长期约束；所有新窗口先读。 |
| `CURRENT_STATE.md` | 根目录动态状态；记录进度、写入锁、阻塞和下一步。 |
...
- 用户原话已记录且只追加。
- 根目录 `AGENTS.md` 已生成或合并，且保留原有仓库规则。
- 根目录 `CURRENT_STATE.md` 已生成，且记录当前任务、写入锁、阻塞和下一步。
- AI 可读需求可追溯到用户原话记录。
- 架构与设计覆盖了当前阶段 / P0 需求。
- 接口契约明确且冻结。
- 总任务可追溯到需求，且颗粒度足够。
